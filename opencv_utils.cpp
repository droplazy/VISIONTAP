...
// 截取屏幕指定区域并返回cv::Mat
cv::Mat capture_screen_area(int x, int y, int width, int height)
{
    // 重新生成背景图片
    snap_screen();

    // 读取截图
    cv::Mat targetImage = cv::imread("/data/machine_vision/background.png");
    ...
    // 使用现有的cropImage函数裁剪指定区域
    return cropImage(targetImage, x, y, width, height);
}
