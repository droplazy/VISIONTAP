...
void ensure_file_permissions() {
    log_message("确保文件权限可执行");
    
    // 检查并设置启动脚本权限
    if (access(LAUNCH_SCRIPT, F_OK) == 0) {
        // 尝试设置为755
        if (chmod(LAUNCH_SCRIPT, 0755) != 0) {
            log_message("设置为755失败，尝试777: %s", strerror(errno));
            chmod(LAUNCH_SCRIPT, 0777);
        }
        log_message("已设置启动脚本权限");
    }
    
    // 检查并设置可执行文件权限
    const char *exe_path = "/vendor/etc/vision/cv_exe";
...
void setup_vision_data() {
    log_message("== 智能数据目录初始化 ==");
    
    const char *data_dir = "/data/machine_vision";
    const char *vendor_dir = "/vendor/etc/vision";
    char cv_exe_path[256];
    
    // 构建cv_exe的完整路径
    snprintf(cv_exe_path, sizeof(cv_exe_path), "%s/cv_exe", data_dir);
    
    // 1. 创建数据目录（如果不存在）
    if (access(data_dir, F_OK) == -1) {
        log_message("创建目录: %s", data_dir);
        char mkdir_cmd[256];
        snprintf(mkdir_cmd, sizeof(mkdir_cmd), "mkdir -p %s", data_dir);
        system(mkdir_cmd);
        system("chmod 755 /data/machine_vision 2>/dev/null");
        log_message("目录创建完成");
    } else {
        log_message("目录已存在: %s", data_dir);
    }
    
    // 2. 检查是否有cv_exe文件
    if (access(cv_exe_path, F_OK) == 0) {
        log_message("发现cv_exe文件，跳过所有操作");
        log_message("== 数据目录初始化完成 ==");
        return;  // 已经有cv_exe，直接返回
    }
    
    log_message("未发现cv_exe文件，需要从vendor拷贝");
    
    // 3. 检查vendor目录是否存在
    if (access(vendor_dir, F_OK) != 0) {
        log_message("错误: vendor目录不存在: %s", vendor_dir);
        log_message("== 数据目录初始化完成 ==");
        return;
    }
    
    log_message("开始从vendor目录拷贝所有内容...");
    
    // 4. 执行拷贝命令
    char cp_cmd[512];
    snprintf(cp_cmd, sizeof(cp_cmd), "cp -rf %s/* %s/ 2>/dev/null", vendor_dir, data_dir);
    
    log_message("执行命令: %s", cp_cmd);
    ...
    if (WIFEXITED(ret)) {
        ...
        if (exit_status == 0) {
            ...
            log_message("文件拷贝完成，但可能有错误（退出码: %d）", exit_status);
        }
    }
    ...
    // 6. 再次检查cv_exe是否存在
    if (access(cv_exe_path, F_OK) == 0) {
        log_message("cv_exe文件已成功拷贝");
    } else {
        log_message("警告: cv_exe文件可能未正确拷贝");
    }
