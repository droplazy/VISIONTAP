...
void Thread_Tikok::CheckReminder()
{
    snap_screen();

    cv::Mat targetImage = cv::imread("/data/machine_vision/background.png");  // 读取目标图像
    cv::Mat templateImage = cv::imread("/data/machine_vision/apppic/donotremind.png"); // 读取模板图像
    double score;
    ad_point match = FindPicTarget(targetImage, templateImage, score);
    if(score >0.8)
    {
...
bool Thread_Tikok::checklogin()
{
    ...
    else
    {
        ...
        {
            cv::Mat templateImage = cv::imread("/data/machine_vision/apppic/loggin_ui_2.png"); // 读取模板图像
            double score;
            ad_point match = FindPicTarget(targetImage, templateImage, score);
            if(score >0.8)
            {
...
bool Thread_Tikok::ShowMyHomepage()
{
    ...
    for (int var = 0; var < 3; ++var) {
        ad_point match = FindTargetForDelay(TIKTOK_HOMEPAGE_CV,score,1);
...
void Thread_Tikok::TaskUpdate(Dev_Action task)
{
    ...
    action = task;
    this->commandid = task.command_id;
    selectTaskPreExec();

    cout << "活动已经更新   --> "  <<   action.sub_action<< endl;
    cout << "状态码   --> "  <<   TASK_EXEC<< endl;
    std::cout << "id: " << this->remark_id << std::endl;
    std::cout << "link: " << this->remark_link << std::endl;
    std::cout << "msg: " << this->remark_msg << std::endl;
    std::cout << "mark: " << this->remark_mark << std::endl;
    std::cout << "commandid: " << this->commandid << std::endl;
...
void Thread_Tikok::parseText(const string &text)
{
    ...
    auto [id, link, msg,mark] = extractor.extractContent(text);
...
bool Thread_Tikok::app_login_2()
{
    ad_point match ,click;
    match.x = 150;
    match.y = 200;
    ...
    if(TASK_EXEC==TASK_LOGIN_CRCODE)
    {
        return captureLoginCRcode();
    }


    for (int var = 0; var < 5; ++var) {
        match = FindTargetReturnPoint("/data/machine_vision/apppic/editoffset_loggin.png");
        usleep(200*1000);
        if(match.x ==-1 || match.y ==-1)
        {
            cout << "没有找到编辑兰  "<< endl;
    ...
    for (int var = 0; var < 5; ++var) {
        ...
        else
        {
            ...
            cout << "点击编辑栏目\n" << endl;
            INPUT_TAP_DELAY(match,500);


            match = FindTargetReturnPoint(TEXTPASTE_PATH);
            usleep(200*1000);
            if(match.x ==-1 || match.y ==-1)
            {
                cout << "PASTE失败  "<<click.x <<", "<<click.y << endl;
                INPUT_TAP_DELAY(click,500);
    ...
    for (int var = 0; var < 5; ++var) {
        match = FindTargetReturnPoint("/data/machine_vision/apppic/get_verifycode_loggin.png");
        usleep(200*1000);
        if(match.x ==-1 || match.y ==-1)
        {
            cout << "找不到验证码的位置  "<< endl;
            match = FindTargetReturnPoint("/data/machine_vision/apppic/agreelogin.png");
            usleep(200*1000);
            if(match.x ==-1 || match.y ==-1)
            {
                cout << "失败 次数 "<< var <<"准备重试" << endl;
                match.x = 500;
                match.y = 340;
                cout << "点击发送演奏马"<<endl;
...
bool Thread_Tikok::app_login_sel()
{
    ad_point match;
    double score =0.0f;
    ...
    for (int var = 0; var < 6; ++var)
    {
        ad_point clickp= TIKTOK_OPT_I;
        INPUT_TAP(clickp);
        match =FindTargetForDelay("/data/machine_vision/apppic/tiktoklogin.png",score,1);
        if(match.x == -1 || match.y == -1)
        {
            match =FindTargetForDelay("/data/machine_vision/apppic/loggin_ui_2.png",score,1);
            if(match.x == -1 || match.y == -1)
            {
                match = FindTargetForDelay(TIKTOK_HOMEPAGE_CV,score,1);
...
bool Thread_Tikok::app_login()
{
    ...
    double score=0.0f;
    //  match = FindTargetForDelay("/data/machine_vision/apppic/needloggin_tiktok.png",score,3);
    // if(match.x == -1 || match.y == -1)
    // {
    //     cout << "没有找到图标[登录]...\n" <<endl;
    ad_point match;
    for (int var = 0; var < 5; ++var)
    {
        ...
        {
            cout << "已经切换到登录界面" << match.x << " ， "  << match.y <<endl;
            break;
        }
    }
    ...
    if(TASK_EXEC==TASK_LOGIN_CRCODE)
    {
        return captureLoginCRcode_2();
    }


    for (int var = 0; var < 5; ++var)
    {
        match = FindTargetForDelay("/data/machine_vision/apppic/loggin_ui.png",score,1);
        if(match.x == -1 || match.y == -1)
        {
            cout << "登录界面不匹配 " << score<< " ， "  << match.y <<endl;
            match.x = 150;
            match.y = 200;
    ...
    for(int i= 0 ; i< 5 ;i++)
    {
        ...
        match = FindTargetReturnPoint("/data/machine_vision/apppic/loggin.png");
        usleep(200*1000);
        if(match.x ==-1 || match.y ==-1)
        {
    ...
    //TASK_EXEC=TASK_LOGIN_WAITING_VERIFY_CODE;
    return true;
}
bool Thread_Tikok::onAppStart()
{
    ...
    if(TASK_EXEC == TASK_LOGIN)
    {
        parseText(action.remark);
        CopyTextFormSys(this->remark_id);
    }

    turnon_application(APP_TIKTOK_ENUM);
    while (!CheckLaunching())
    {
        SHORT_DELAY;
    ...
    if(TASK_EXEC == TASK_LOGIN || TASK_EXEC == TASK_LOGIN_CRCODE)
    {
            ...
            {
                applacationstate = AppState::NEEDLOGIN;
                return false;
            }
    ...
    else
    {
        ...
        {
            if(applacationstate != AppState::NEEDLOGIN)
            applacationstate = AppState::ERROR;
            return false;
        }
    }
...
bool Thread_Tikok::hasTask() //这是轮询标志
{
    ...
    if(TASK_EXEC==TASK_COMPELTED || TASK_EXEC == TASK_UNKNOW||TASK_EXEC == TASK_UNKNOW)
    {
...
void Thread_Tikok::checklvingroomwid()
{

        double score;
    ad_point match = FindTargetForDelay("/data/machine_vision/apppic/livingwid.png",score,1);
    if(match.x == -1 || match.y == -1)
    {
...
void Thread_Tikok::executeTask()
{
    ...
    if(TASK_EXEC==TASK_COMPELTED || TASK_EXEC==TASK_EXEC_FAILD)
    {
        applacationstate = AppState::EXITING;
        cout << "等待外部回收线程 " << threadName_ <<endl;
    }
    else if(TASK_EXEC == TASK_SEND_MESSAGE)
    {
    ...
    else if(TASK_EXEC == TASK_FOLLOW_MODE_RUNNING)
    {
        ...
        for (i = 0; i < 3; ++i)
        {
            ...
            {
                cout << "buzai zhi bojianle  >>>......\n" << endl;
                TASK_EXEC = TASK_EXEC_FAILD;
            }
        }
        if(!ProhibitFollow_b)
        {
            if(RandomFollowUser() == -3)
            {
    ...
    else if(TASK_EXEC ==  TASK_LOGIN)
    {
        std::this_thread::sleep_for(std::chrono::seconds(1));
        std::cout << "等待验证码下发"  << std::endl;

    }
    else if(TASK_EXEC ==  TASK_LOGIN_CRCODE)
    {
        std::cout << "等待扫码"  << std::endl;
        ...
        double score = 0.0f;
        ad_point match = FindTargetForDelay("/data/machine_vision/apppic/unvaildCRcode.png",score,1);
        if(match.x >0 || match.y >0)
        {
            std::cout << "二维码已经实效" <<  score << std::endl;

            TASK_EXEC =TASK_EXEC_FAILD;
            return ;
        }
        std::cout << "二维码" <<  score << std::endl;

    }
    else if(TASK_EXEC ==  TASK_LOGIN_VERIFY_CODE)
    {
        ad_point  match;
        // match.x = 150;
        // match.y = 200;
        ...
        inputNumber(remark_msg );

        double score =0.0f;
        for(int i=0;i<2;i++)
        {
            match = FindTargetForDelay("/data/machine_vision/apppic/itsme.png",score,3);
            ...
            {
                cout <<" 需要点击确认 点击"  <<match.x<<"," <<match.y<<".\n" <<endl;
                INPUT_TAP(match);
            }
        }
        ...
        for(int i=0;i<2;i++)
        {
            match = FindTargetForDelay("/data/machine_vision/apppic/verify_face.png",score,3);
...
int Thread_Tikok::SendBraggerForLivingRoom(string message,bool noEdit)
{
    ...
    for( var = 0; var < 5; ++var)
    {
        ...
        match = FindTargetForDelay(TIKTOK_LIVING_KEYBOARD_UI_CV,score,3);
        cout << "等待键盘弹出.1..\n" <<endl;
        ...
        if(match.x == -1 || match.y == -1)
        {
            clickP = TIKTOK_OPT_BARRAGE_BUTTON;
            INPUT_TAP(clickP);
            match = FindTargetForDelay(TIKTOK_LIVING_KEYBOARD_2_UI_CV,score,3);
            if(match.x == -1 || match.y == -1)
            {
...
int Thread_Tikok::SendMessageToPerson(string name,string message)
{
    ...
    double score =0.0f;
    ad_point match = FindTargetForDelay(TIKTOK_SEND_MESSAGE_CV,score,5);
    if(match.x == -1 || match.y == -1)
    {
        cout << "未找到发送信息...\n" <<endl;
    }
    ...
    {
        cout << "点击发送信息给个人.."  <<match.x<<"," <<match.y<<".\n" <<endl;
    }
    INPUT_TAP(match);

    match = FindTargetForDelay(TIKTOK_SEND_MESSAGE_UI_2_CV,score,5);
    if(match.x == -1 || match.y == -1)
    {
        cout << "未找到发送信息按钮...\n" <<endl;
    }
    ...
    {
        match.x -=200;
        cout << "找到发送按钮，准备点击编辑栏目"  <<match.x<<"," <<match.y<<".\n" <<endl;
    }
    INPUT_TAP(match);
    ...
    {
        match.x -=200;
        cout << "找到发送按钮，准备点击编辑栏目222"  <<match.x<<"," <<match.y<<".\n" <<endl;
    }
    INPUT_TAP_DELAY(match,500);
    //点击粘贴ints
...
void Thread_Tikok::selectTaskPreExec()
{
    ...
    else if (action.sub_action == "登录")
    {
        TASK_EXEC = TASK_LOGIN;
    }
    else if (action.sub_action == "二维码登录")
    {
        TASK_EXEC = TASK_LOGIN_CRCODE;
    }
    else if (action.sub_action == "验证码")
    {
        TASK_EXEC = TASK_LOGIN_VERIFY_CODE;
    }
    else
    {
...
bool Thread_Tikok::captureLoginCRcode()
{

    cv::Mat cropImage =   capture_screen_area(230,185,200,200);
    double score =0.0f;
    cv::Mat findIMg = cv::imread("/data/machine_vision/apppic/vaildCRcode.png"); // 读取模板图像
    for (int var = 0; var < 10; ++var) {


        ad_point match= FindPicTarget(findIMg, cropImage, score);
        if(match.x == -1 || match.y == -1)
        {
            ad_point click ;
            click.x = 320;
            ...
            cout << "点击刷新"<<endl;
            SHORT_DELAY;
            cropImage =   capture_screen_area(230,185,200,200);
        }
        else
        {
            cout << "二维码正确 "<< score << endl;
            cout << "二维码正确 "<< score << endl;
            string picPath = "/data/machine_vision/vaild_CRcode.jpg";
            cv::imwrite(picPath,cropImage);
            TASK_EXEC= TASK_LOGIN_CRCODE;
            string t_ip =IP_HOST;
            string url ="http://"+t_ip+":8080dev/upload?commandid="+this->commandid;
...
bool Thread_Tikok::captureLoginCRcode_2()
{
    ad_point click ;
    click.x = 150;
    click.y = 200;
    ...
    for (int var = 0; var < 10; ++var) {
        INPUT_TAP(click);
        cout << "点击扫码"<<endl;
        cv::Mat cropImage =   capture_screen_area(404,182,210,210);
        double score =0.0f;
        cv::Mat findIMg = cv::imread("/data/machine_vision/apppic/vaildCRcode.png"); // 读取模板图像
        ...
        else
        {
            ...
            string url ="http://"+t_ip+":8080/dev/upload?commandid="+this->commandid;
            Uploadfile(url,picPath);
            return true;
        }
    }
